package com.tadams.tcars.ui.theme

import androidx.compose.ui.unit.dp

// https://archive.ph/k1zdm

val HORIZONTAL_BAR_HEIGHT = 16.dp // 30, 0.5rem
val INTRA_FRAME_GAP = 2.dp // 5,
val INTER_FRAME_GAP = 4.dp
val SURFACE_PADDING = 4.dp
val LinearIndicatorHeight = 16.dp
val LinearIndicatorUnselectedHeightFraction = .66f
val SWEPT_CORNER_RADIUS = 64.dp // 75, 160
val CONTENT_CORNER_RADIUS = 28.dp // 37.5, 60
val CONTENT_PADDING = 12.dp

val BIG_BUTTON_WIDTH = 80.dp // 150,
val HORIZONTAL_BAR_MIN_WIDTH = 8.dp

val MIN_LABEL_WIDTH = 27.dp
val LABEL_PADDING = 8.dp